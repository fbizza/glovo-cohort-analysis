...
def get_orders_for_period(period_start_date, period_end_date):
    sql_query = f"""
        WITH orders AS (
        SELECT o.order_id,
            DATE (o.order_started_local_at) AS DATE,
            o.store_name,
            o.order_vertical,
            o.customer_id,
            p.discount_subtype
...
def process_queries(timeframe, n_periods=3):
    ...
    with open(f'../data/promo_data/{timeframe}/recurrent_customers.pkl', 'wb') as file:
        pickle.dump(recurrent_customers, file)
    with open(f'../data/promo_data/{timeframe}/orders.pkl', 'wb') as file:
